...
export function DueDateDialog() {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[200] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-[320px] overflow-hidden border dark:border-slate-700">
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center mb-4">
            <button onClick={prevMonth} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300">◀</button>
            <span className="font-bold text-slate-700 dark:text-slate-200">{MONTH_NAMES[currentMonth]} {currentYear}</span>
            <button onClick={nextMonth} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-600 dark:text-slate-300">▶</button>
          </div>

          {/* Days Grid */}
          <div className="grid grid-cols-7 gap-1 text-center text-xs mb-2 text-slate-400 dark:text-slate-500 font-bold uppercase">
                ...
                <button
                  ...
                  className={`
                    ...
                    ${selected 
                      ...
                      : today 
                        ? "text-indigo-600 dark:text-indigo-400 font-black bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800" 
          ...
          {/* Action Buttons */}
          <div className="flex flex-col gap-2 mt-6 border-t dark:border-slate-700 pt-4">
            <div className="flex gap-2">
              ...
              <button 
                onClick={handleClear}
                className="px-4 py-2 text-slate-500 dark:text-slate-400 font-bold hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition"
              >
                Clear
              </button>
            </div>
